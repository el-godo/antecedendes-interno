...
$MM_restrictGoTo = "../paginas/error.php";
...
$colname_ConsultaDNI = "-1";
if (isset($_POST['dni'])) {
  $colname_ConsultaDNI = $_POST['dni'];
}
mysql_select_db($database_antecedentes, $antecedentes);
$query_ConsultaDNI = sprintf("SELECT DocumentoNro FROM padron WHERE DocumentoNro = %s", GetSQLValueString($colname_ConsultaDNI, "text"));
$ConsultaDNI = mysql_query($query_ConsultaDNI, $antecedentes) or die(mysql_error());
$row_ConsultaDNI = mysql_fetch_assoc($ConsultaDNI);
$totalRows_ConsultaDNI = mysql_num_rows($ConsultaDNI);
?>
...
<ul>
	...
	<div id="content">
		...
		<div id="main">
		  <div class="full_w"><!-- InstanceBeginEditable name="EditRegion1" --><div class="h_title">Alta de Solicitante</div>
          <div class="Table">
              <?php 
			  // If para consultar si el usuario existe en el sistema. Si existe no lo da de alta.
			  if ($totalRows_ConsultaDNI == 0) { 
	  ...
	  mysql_query("INSERT INTO padron (DocumentoNro, ProntuarioNro, Apellido, Nombre, Antecedente) VALUES ('$documento', '$prontuario', '$apellido', '$nombres', '$antecedentes') ");
?>
			<div>
			<h3 align="center">Los datos se ingresaron correctamente.</h3>
			<p>&nbsp;</p>
			<h3 align="center">Proceda a <a href="../paginas/entrada/index.php">Solicitar el Certificado</a>.</h3>
        <?php } else { ?>
			<div><h3 align="center">No se puede dar de alta a la Persona debido a que ya existe en el Sistema.</h3>
            <div>
              <h3 align="center">Intente Generar la solicitud: <a href="../paginas/entrada/index.php">Alta de Solicitud</a></h3>
			<?php }?>

<p align="center">&nbsp;</p>
          </div></div><head><!-- InstanceEndEditable --></div>
		</div>
		<div class="clear"></div>
	</div>
