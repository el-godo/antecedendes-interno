...
if (isset($_POST['DocumentoNro'])) {
  $colname_Busqueda = $_POST['DocumentoNro'];
}
mysql_select_db($database_antecedentes, $antecedentes);
...
<?php
// Si no hay solicitudes pendientes pasar ejecutar el form4 q envia los datos a generar certificado.
if ($totalRows_Solicitud == 0) { // Show if recordset empty ?>
  <form action="solicitud_procesar.php" method="post" id="form4">
    <input name="DocumentoNro" type="hidden" id="DocumentoNro" value="<?php echo $_POST["DocumentoNro"]; ?>" />
  </form>
...
<div class="h_title">Verificación de Solicitante</div>
          <div class="Table">
            ...
            <p>
<?php if (isset($_POST["DocumentoNro"])) { // Show if recordset not empty ?></p>
		...
		if ($totalRows_Solicitud == 0) { // Show if recordset empty ?>
                        <form id="form1" name="form1" method="post" action="solicitud_procesar.php">
                          ...
                          <input name="DocumentoNro" type="hidden" id="DocumentoNro" value="<?php echo $_POST["DocumentoNro"]; ?>" />
                          <input name="button" type="submit" autofocus="autofocus" id="button" value="Generar Certificado" />
                        </form>
                        <?php } // Show if recordset empty ?>
                      <?php 
...
//".mysql_error());
?>
                <span class="ui-state-error">El solicitante DNI Nro: <?php echo $documento; ?> no se encuentra en la base de datos. Proceda a dar de alta.</span>
            </h3>
              <form id="form2" name="form2" method="post" action="alta_solicitante.php">
			    <input type="submit" name="button2" id="button2" value="Alta de Solicitante" />
                            <input name="txtbuscar" type="hidden" id="txtbuscar" value="<?php echo $documento; ?>" />
		    </form>
						  <p>&nbsp;</p> 
							<p>
							  <?php  } ?>
...
<h3 class="ui-state-error">Debe completar el proceso de impresi&oacute;n o Anular la Solicitud.</h3>
            ...
            <table border="1">
              ...
              <?php do { ?>
                <tr>
                  ...
                  <td><?php
				  // Mostrar cuando las solicitudes sean igual a 1
                  if ($totalRows_Solicitud == 1) { // Show if recordset empty ?>
                      <form id="form3" name="form3" method="post" action="solicitud_procesar.php">
                        <input name="documento" type="hidden" id="documento" value="<?php echo $row_Busqueda['DocumentoNro']; ?>" />
                        <input name="prontuario" type="hidden" id="prontuario" value="<?php echo $row_Busqueda['ProntuarioNro']; ?>" />
                        <input name="apellido" type="hidden" id="apellido" value="<?php echo $row_Busqueda['Apellido']; ?>" />
                        <input name="nombres" type="hidden" id="nombres" value="<?php echo $row_Busqueda['Nombre']; ?>" />
                        <input name="DocumentoNro" type="hidden" id="DocumentoNro" value="<?php echo $row_Solicitud["dni"]; ?>" />
                        <input name="id_solicitud" type="hidden" id="id_solicitud" value="<?php echo $row_Solicitud["id_solicitud"]; ?>" />
                        <input name="button" type="submit" autofocus="autofocus" id="button" value="Generar Certificado" />
                      </form>
                      <?php } // Show if recordset empty ?>
              ...
              <input name="DocumentoNro" type="hidden" value="<?php echo $_POST['DocumentoNro']; ?>" />
